...
你给出的三折思路与论文“外层新鲜 holdout”思想一致；本项目最终按你的后续要求改为 5 折外层。

## 2. 本项目约束下的 NCV 目标

- 任务：沿用 Simulation 1 的 16 个候选子模型做选模正确率比较。
- 外层：5-fold（每轮 1 折 holdout）。
- 内层：在剩余 4 折上做 4-fold CV 以选模型。
- 关键约束：每一轮内层选模不能使用该轮 holdout 的任何信息。

## 3. 方法定义（含你要求的 ncv1 / ncv2 / ncv3）

记外层五轮得到五组候选 `(m1,...,m5)`，对应外层 holdout 误差 `(e1,...,e5)`。
...
- `outer_best`：五轮外层误差 `e1,...,e5` 中最小者对应模型。
- `outer_1se`：在 `min(e) + 1SE_outer` 范围内，选“正则化更强”的模型。
  这里 `1SE_outer` 用五轮误差的样本标准差除以 `sqrt(5)`。
...
- 风险点：
  - 外层虽增至 5 轮，但 `outer_1se` 仍可能有波动；需在报告中注明。
