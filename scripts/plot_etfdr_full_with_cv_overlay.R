...
main <- function() {
  curve_path <- file.path("results", "etfdr_fig1_ncv1_overlay_curve_overlay.csv")
  sel_path <- file.path("results", "etfdr_fig1_ncv1_overlay_all_selected.csv")
  out_path <- file.path("figures", "etfdr_fig1_ncv1_overlay_with_cv_figure.png")

  if (!file.exists(curve_path) || !file.exists(sel_path)) {
    stop("Missing full results. Please run full first.")
  }
